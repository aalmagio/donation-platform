...
//202504260930
/*
 * 2025-04-26 Aggiunta gestione Referral code 
 */
if ( !isset( $_SESSION ) ) {
    session_start();
}
...
if ( isset( $_GET[ 'date3' ] ) && isset( $_GET[ 'date4' ] ) ) {
    if ( $_GET[ 'date3' ] > date( "Y-m-d" ) ) {
        ...
        if ( isset( $_GET[ 'ID_Mentor' ] ) && $_GET[ 'ID_Mentor' ] == "NOTNULL" ) { //SCRITTE IN MENTOR
            ...
            $query_donazione = sprintf( "SELECT Donazione.Id_a, Donazione.CodTrans, Donazione.importo, Donazione.data, Donazione.pay_method, Donazione.tipo, Donazione.CodiceMentor, Donazione.valido,
                    Anagrafica.ID_Mentor, Anagrafica.nome, Anagrafica.cognome, Anagrafica.ragioneSociale, Anagrafica.CodiceReferral,
                    (
                      SELECT COUNT(*)
                      FROM Anagrafica AS A2
                      LEFT JOIN Donazione as D2
                      ON A2.Id_a = D2.Id_A
                      WHERE A2.CodiceReferral =Anagrafica.CodicePersonale
                      and D2.Esito ='OK'  
                    ) AS NumeroUtilizziCodicePersonale
                    FROM Donazione
            ...
            $query_donazione = sprintf( "SELECT 
                    Donazione.Id_a, Donazione.CodTrans, Donazione.importo, Donazione.data, Donazione.pay_method, Donazione.tipo, Donazione.CodiceMentor, Donazione.valido,
                    Anagrafica.ID_Mentor, Anagrafica.nome, Anagrafica.cognome, Anagrafica.ragioneSociale, Anagrafica.CodiceReferral,
                    (
                      SELECT COUNT(*)
                      FROM Anagrafica AS A2
        ...
        <main role="main" class="col-md-9 ml-sm-auto col-lg-10 px-md-4">
            ...
            <div class="table-responsive">
                <table class="table table-striped table-sm">
                    ...
                    <tbody>
                        <?php do{ ?>
                        <tr <?php if ($row_donazione['valido'] == 'N'){ echo' class="bg-secondary text-white"';}?> >
                            <?php if ($_SESSION['MM_UserGroup'] =="A"){ ?>
                            ...
                            <?php if (USE_MENTOR == true) { ?>
                            <td><?php if ($row_donazione['ID_Mentor']==""){  echo stripslashes($row_donazione['CodTrans']); } else {?>
                                <a href="singola.php?CodTrans=<?php echo stripslashes($row_donazione['CodTrans']); ?>"> <?php echo stripslashes($row_donazione['CodTrans']); ?> </a>
                                <?php } ?></td>
                            <?php } else{ ?>
                            <td><a href="singola.php?CodTrans=<?php echo stripslashes($row_donazione['CodTrans']); ?>"> <?php echo stripslashes($row_donazione['CodTrans']); ?> </a></td>
                            <?php } ?>
                            ...
                            if ( $row_donazione[ 'CodiceReferral' ] == "" ) {
                                ...
                                if ( $totalRows_referral == 1 ) {
                                    echo "<strong style='color:#67191c;'>" . $row_referral[ 'nome' ] . " " . $row_referral[ 'cognome' ]."</strong>";
                                } else {
                            ...
                            ?></td>
                            <td> <?php echo $row_donazione['NumeroUtilizziCodicePersonale']; ?> </td>
                            <?php if (USE_MENTOR == true) { ?>
                            ...
                            <td colspan="2" style="text-align: center;"><?php if ($row_donazione['ID_Mentor']==""  && ($_SESSION['MM_UserGroup'] =="A" ||$_SESSION['MM_UserGroup'] =="S")){ ?>
                                <a href="singola.php?CodTrans=<?php echo stripslashes($row_donazione['CodTrans']); ?>"><strong>Scrivi in Mentor</strong></a>
                                <?php } else { ?>
                        ...
                        <tr>
                            ...
                            <td>&nbsp;</td>
                            <td>&nbsp;</td>
                            <td><a href="export-xls.php" target="_blank">Scarica tutte le donazioni</a></td>
                        </tr>
                    </tbody>
                </table>
            </div>
            ...
            <form method="get" action="<?php echo $_SERVER['PHP_SELF']; ?>" name="Ricerca_periodo">
                ...
                <div style="clear:both;">
                    ...
                    <p>
                        <label for="ID_Mentor">Tipo Donazioni :</label>
