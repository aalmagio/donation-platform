...
if ( isset( $_POST[ "MM_update" ] ) && $_POST[ "MM_update" ] == "mod_esito" ) {
    $conn = mysqli_connect( DB_IP, DB_USER, DB_PASSWORD, DB_DBNAME )or trigger_error( mysqli_error(), E_USER_ERROR );

    $updateSQL = sprintf( "UPDATE `Donazione` SET `esito` = %s WHERE `Donazione`.`Id_a` = %s AND `Donazione`.`CodTrans` = %s ;",
        GetSQLValueString( strtoupper( $_POST[ 'esito' ] ), "text" ),
        GetSQLValueString( $_POST[ 'Id_a' ], "int" ),
        GetSQLValueString( $_POST[ 'CodTrans' ], "text" ) );
    ...
    <div class="row">
        ...
        <main role="main" class="col-md-9 ml-sm-auto col-lg-10 px-md-4">
            ...
            <div class="container-fluid mt-3">
                ...
                <form action="<?php echo $editFormAction; ?>" method="post" name="mod_esito" class = "form-horizontal" role = "form">
                    <div class="form-group">
                        <label for="esito" class="control-label col-sm-2">Esito: </label>
                        <div class="col-sm-6">
                            <select name="esito" class="form-control">
                                <option value="OK" <?php if ($row_donazione['esito'] =="OK") echo "selected=\"selected\""; ?>>OK</option>
                                <option value="KO" <?php if ($row_donazione['esito'] =="KO") echo "selected=\"selected\""; ?>>KO</option>
                                <option value="WA" <?php if ($row_donazione['esito'] =="WA") echo "selected=\"selected\""; ?>>WA</option>
                                <option value="RF" <?php if ($row_donazione['esito'] =="RF") echo "selected=\"selected\""; ?>>RF</option>
                            </select>
                        </div>
                        <div class="form-group">
                            <div class="col-sm-2">
                        ...
                        <input type="hidden" name="MM_update" value="mod_esito" />
                        <input type="hidden" name="Id_a" value="<?=  $row_donazione['Id_a']; ?>">
                        <input type="hidden" name="CodTrans" value="<?=  $row_donazione['CodTrans']; ?>">
                    </div>
                </form>
            </div>
            <hr>
            ...
            if ( isset( $Result1 ) ) {
                echo "<p class=\"alert alert-success \" role=\"alert\">L'esito della donazione &egrave; stato modificato. </p>";
